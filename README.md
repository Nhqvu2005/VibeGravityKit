...
> **Solution:** Persistent team profiles that auto-inject your DNA into every new project.

### How It Works

```bash
# Step 1: Create a team by scanning your existing project
vibegravity team create my-team --scan ./my-best-project

# Step 2: Init any new project with your team
vibegravity init antigravity --team my-team
```

That's it. Your new project now knows your coding style, preferred stack, naming convention, and all lessons learned from past projects.
...
```
naming:camelCase|comments:minimal|stack:react+vite+tailwind|be:fastapi|arch:feature|indent:2
```

This compact format (~50 tokens) tells every agent exactly how you like your code. Auto-generated by scanning your existing projects.

### What Gets Detected (via Code Scanner)
...
# AI-assisted learning (run inside IDE)
vibegravity team learn                        # Scan conversations → extract habits
```

### Auto-Learn (End of Session)

After each session, agents auto-detect repeated directives and add them to your team profile:
```
Detected: "Write all docs in English" (appeared 3 times) → added to Hot rules
Detected: "Always commit + push" (appeared 5 times) → added to Hot rules
Team DNA updated: ...lang:en|commit:always-push...
```

### Bug Fix Knowledge Transfer

Journal entries from past projects auto-inject into new ones:
```
Project A: Fixed "CORS issue with Vite proxy" → saved to team
Project B: Agent encounters CORS → searches journal → finds solution → fixes instantly
```

No agent ever encounters the same bug twice.
...
| `vibegravity team create <name> --scan <path>` | Create team from code scan |
